...
\begin{proc}{esPeriodico}{\In t: $toroide$, \Inout p: $\ent$, \Out result: $\bool$}{}
    \pre{esToroideValido(t)}
    \post{result = \True \Iff (\exists ks: \TLista{\ent})(length(ks) > 0 \yLuego ks[0] > 0 \yLuego ordenadaDeMenorAMayor(ks) \yLuego esListaDePeriodicos(t, ks) \yLuego p = ks[0]}

    \pred{esListaDePeriodicos}{t: $toroide$, ks: $\TLista{\ent}$}{
        \\(\exists ts: \TLista{toroide})(length(ts) = length(ks) \yLuego esEvolucion(t, ts[0]) \\\yLuego (\forall i: \ent)((0 \leq i < length(ts))\implicaLuego esEvolucionMultiple(t, ts[i], ks[i])))
    }
\end{proc}
...
\begin{proc}{seleccionNatural}{\In ts: $\TLista{toroide}$, \Out res: $\ent$}{}
    ...
    \post{(\exists is: \TLista{\ent})(length(is) = length(ts) \\\yLuego ordenadaDeMayorAMenorPorCantidadDeTicksHastaMuerte(ts, is) 
    \\\yLuego res = is[0])}
    ...
    \pred{ordenadaDeMayorAMenorPorCantidadDeTicksHastaMuerte}{ts: $\TLista{toroide}$, is: $\TLista{\ent}$}{
        (\forall i: \ent)(((i \in is)\yLuego(0 < i < length(is)))\\\implicaLuego(cantidadDeTicksHastaMuerte(ts[i - 1]) \geq cantidadDeTicksHastaMuerte(ts[i])))
    }

    \aux{cantidadDeTicksHastaMuerte}{t: $toroide$}{\ent} {
        \\\IfThenElse{(\exists tx: $toroide$)(mismaDimension(t, tx) \yLuego cantidadVivas(tx) = 0)\yLuego(\exists ks: \TLista{\ent})(length(ks) > 0 \yLuego ks[0] > 0 \yLuego ordenadaDeMenorAMayor(is) \yLuego esEvolucionMultiple(t, tx, ks[i - 1]))}{
            ks[0]
        \\}{
            -1
        }
...
\pred{ordenadaDeMenorAMayor}{ks: $\TLista{\ent}$}{
        (\forall i: \ent)((0 < i < length(ks))\implicaLuego(ks[i - 1] < ks[i]))
    }
